...
| run15   | `341b71f`     | **杀噪声梯度**：修 `scale_rewards` 静默 bug（`"none"` 是 truthy，之前一直在除 std 放大噪声）；加 `filter_zero_std_groups` + `filter_zero_correct_groups` 把无信号 group 的 advantage 和 completion_mask 同时清零；加 `log_rollouts_every=100` sanity-check 解析器 |    48 |       0.0 |         — |     ~0.01 |       0.0 |   0.71 | rollout dump 暴露根因：LLaDA 从不输出 `<answer>…</answer>`，直接写自然语言答案；旧 `extract_xml_answer` 无 tag 时返回整段 completion → correctness 恒为 0 → filter 饱和到 0.87–1.0 → loss=grad=0 |
| run16   | `ad49fcd`     | 修 **答案解析 bug**：新增 `extract_answer_lenient`（优先 `<answer>`，其次 `#### N`、`\boxed{…}`、最后"最后一个数字"），correctness_reward_func 改用 lenient 解析 + 两侧归一化（去逗号、去 `.0`）；strict `extract_xml_answer` 保留给 int/format 奖励。其余配置继承 run15 |    13 |      ~0.1 |     2-4（噪） |     0.6   |     ~3-7  |   0.75 | parser 生效：correctness mean 从 ~0.01 跳到 0.25–1.25（50×↑）；但 `correctness × 5.0` 权重让 correctness 占 27:1 压过格式，且 siblings 太相关 → `frac_groups_filtered ~0.9`；13 步后重平衡权重 → run17 |
| run17   | `f2ff3af`     | **重平衡奖励权重**：`[0.25 0.25 0 0.25 5.0]` → `[1.0 1.0 0 1.0 2.0]`。全对 rollout 权重从 10.0 → 4.0，format 三条合计 0.375 → 1.5，比例从 27:1 降到 2.7:1，让格式信号不再被 correctness 淹没 |   360 |     ~0.00 |     ~1.5 (平) |   ~0.55 (平) |     ~2.3 |  0.4 | reward/correctness 全程无趋势；fork_frac 前 150 步饱和 0.75–0.92，后半段过冲掉到 0.03–0.15（bang-bang）；`frac_groups_filtered` MA20 ~0.97→0.83；根因：ForkHead `value_coupling=1.0` init + V(z) MSE 拟合到 reward ≈ 1.5 把 raw 顶到 1.5 → sigmoid→0.82 → siblings 太像 → group 全被 filter |
| run18   | 本次           | **修 ForkHead 偏置 bug**：`value_coupling` init `1.0 → 0.0`。t=0 时 `mean = sigmoid(proj.bias) = 0.5`，不再被 V(z) 初始拟合的 reward 量级一脚踢到 0.8；REINFORCE 仍可自由地把 coupling 拉正/负 |   跑中 |         — |           — |         — |         — |      — | —                                                |
