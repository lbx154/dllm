...
| run15   | _pending_     | **杀噪声梯度**：修 `scale_rewards` 静默 bug（`"none"` 是 truthy，之前一直在除 std 放大噪声）；加 `filter_zero_std_groups` + `filter_zero_correct_groups` 把无信号 group 的 advantage 和 completion_mask 同时清零；加 `log_rollouts_every=100` sanity-check 解析器 |   跑中 |         — |           — |         — |         — |      — | —                                                |
