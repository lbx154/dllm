...
| run16   | `f509192`         | 修 **答案解析 bug**：新增 `extract_answer_lenient`（优先 `<answer>`，其次 `#### N`、`\boxed{…}`、最后"最后一个数字"），correctness_reward_func 改用 lenient 解析 + 两侧归一化（去逗号、去 `.0`）；strict `extract_xml_answer` 保留给 int/format 奖励。其余配置继承 run15 |   跑中 |         — |           — |         — |         — |      — | —                                                |
