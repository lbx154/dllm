#!/usr/bin/env bash
# Vanilla GRPO run2 — same as run1 but with --loss_type grpo
#
# loss_type='grpo' = original DeepSeek GRPO formulation:
#   loss = mean over each sequence's tokens, then mean over sequences in batch
#   ⚠ has known length bias: prefers SHORT completions with positive advantage
#     and LONG completions with negative advantage (per TRL docs).
#   Loss values are smaller / more interpretable than DAPO (each seq is mean-
#   normalized first, so per-token-sum scale doesn't dominate).
#
# Compare to run1 (loss_type=dapo):
#   - run1 step 1:    loss=1.3e8  (KL-display dominates)
#   - run1 step 324:  loss=9.5    (KL settled; DAPO healthy)
#   - run2 expected:  loss values 1-2 OoM smaller across the board
#
# Purpose:
...
accelerate launch \
    ...
    --load_in_4bit True \
    --dataset gsm8k \
    --output_dir .models/llada-grpo-vanilla-run2 \
    --lora_r 128 \
    --lora_alpha 64 \
    --lora_dropout 0.05 \
    --bf16 True \
    ...
    --save_total_limit 3 \
    --seed 48 \
    --run_name grpo-vanilla-run2-1node \
    --report_to none
